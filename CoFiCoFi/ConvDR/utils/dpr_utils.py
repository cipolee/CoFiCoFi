...
def get_optimizer_defferentlr(args, model: nn.Module, weight_decay: float = 0.0, ):
    no_decay = ['bias', 'LayerNorm.weight']
    param_optimizer = list(model.named_parameters())
    # param_pre = [(n, p) for n, p in param_optimizer]
    for n, p in param_optimizer:
        if 'retriever' in n and 'reader' in n:
            print(n)
        elif 'retriever' not in n and 'reader' not in n:
            print(n)
    # retriever model param
    param_pre = [(n, p) for n, p in param_optimizer if 'retriever' in n]
    # reader model param
    param_downstream = [(n, p) for n, p in param_optimizer if 'reader' in n]
