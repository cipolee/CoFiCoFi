...
#################################xbli
def convert_example_to_feature_with_history(example, tokenizer, max_seq_length=512,
                                 doc_stride=384, max_query_length=125, is_training=True,
                                 cls_token_at_end=False,
                                 cls_token='[CLS]', sep_token='[SEP]', pad_token=0,
                                 sequence_a_segment_id=0, sequence_b_segment_id=1,
    ...
    tok_start_position = None
    tok_end_position = None
    tok_start_position_his = None
    tok_end_position_his = None
    if is_training and example.is_impossible:
        tok_start_position = -1
        tok_end_position = -1
        tok_start_position_his = -1
        tok_end_position_his = -1
    if is_training and not example.is_impossible:
        if 0 <= example.start_position_his <= example.end_position_his <len(orig_to_tok_index):
            tok_start_position_his = orig_to_tok_index[example.start_position_his]
            if example.end_position_his < len(example.doc_tokens) - 1:
                tok_end_position_his = orig_to_tok_index[example.end_position_his + 1] - 1
            else:
                tok_end_position_his = len(all_doc_tokens) - 1
            (tok_start_position_his, tok_end_position_his) = _improve_answer_span_his(
                all_doc_tokens, tok_start_position_his, tok_end_position_his, tokenizer,
                example.last_answer_text)
        else:
            tok_start_position_his, tok_end_position_his = -1,-1
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
        ...
        if is_training and not span_is_impossible:
            ...
            if not (tok_start_position_his >= doc_start and
                    tok_end_position_his <= doc_end):
                out_of_span = True
            if out_of_span:
                start_position_his = -1
                end_position_his = -1
                span_is_impossible = True
            else:
                if sequence_a_is_doc:
                    ...
                    doc_offset = len(query_tokens) + 2
                start_position_his = tok_start_position_his - doc_start + doc_offset
                end_position_his = tok_end_position_his - doc_start + doc_offset
        ...
        feature = InputFeatures(
                    ...
                    is_impossible=span_is_impossible, 
                    retrieval_label=example.retrieval_label,
                    start_position_his=start_position_his,
                    end_position_his=end_position_his,
                    )

    return feature


def gen_reader_features_with_history(qids, question_texts, answer_texts, answer_starts, passage_ids,
                        passages, all_retrieval_labels, reader_tokenizer, max_seq_length,qid_to_last_answer_positions,last_answer_text, is_training=False):
    # print('all_retrieval_labels', all_retrieval_labels, type(all_retrieval_labels))
    batch_features = []
    all_examples, all_features = {}, {}
    for (qas_id, question_text, answer_text, answer_start, pids_per_query,
        ...
        for i, (pid, paragraph_text, retrieval_label) in enumerate(zip(pids_per_query, paragraph_texts, retrieval_labels)):
            ...
            if is_training:
                ...
                if not is_impossible:
                    ...
                    end_position = char_to_word_offset[answer_offset +
                                                       answer_length - 1]
                    start_position_his = qid_to_last_answer_positions[0]
                    end_position_his = qid_to_last_answer_positions[1]
                    if 0<=start_position_his<=end_position_his<len(char_to_word_offset):
                        start_position_his = char_to_word_offset[start_position_his]
                        end_position_his = char_to_word_offset[end_position_his]
                    else:
                        start_position_his,end_position_his=-1,-1
                    # Only add answers where the text can be exactly recovered from the
                    # document. If this CAN'T happen it's likely due to weird Unicode
                    # stuff so we will just skip the example.
                    #
            ...
            feature = convert_example_to_feature_with_history(
                example, reader_tokenizer, is_training=is_training)
            ...
            if is_training:
                if retrieval_label:
                    per_query_feature = {'input_ids': np.asarray(feature.input_ids),
                                     ...
                                     'end_position': feature.end_position + i * max_seq_length,
                                     'retrieval_label': feature.retrieval_label,
                                     'start_position_his':feature.start_position_his + i * max_seq_length if feature.start_position_his and \
                                          feature.start_position_his != -1 else  -1,
                                     'end_position_his':feature.end_position_his + i * max_seq_length if feature.end_position_his  and \
                                          feature.end_position_his != -1 else  -1}
...
def _improve_answer_span_his(doc_tokens, input_start, input_end, tokenizer,#少许修改
                         orig_answer_text):
