...
class RetrieverDataset(Dataset):
    def __init__(self, filename, tokenizer, 
                 ...
                 given_passage=False, given_negative=False,
        ...
        self._given_negative = given_negative
    ...
    def __getitem__(self, idx):
        ...
        if self._given_query:
            ...
            query_feature_dict = {'query_input_ids': np.asarray(query_feature.input_ids), 
                                  ...
                                  'qid': qas_id,
                                  'query_len':sum(query_feature.attention_mask)}
        ...
        if self._given_negative:
            passages = entry['evidences']

            negative_idx = np.argmin(retrieval_labels)
            passage = passages[negative_idx]

            example_id = '{}_{}'.format(qas_id, negative_idx)
            passage_example = RetrieverInputExample(
                                    guid=example_id,
                                    text_a=passage,
                                    label=0)
            ...
            passage_feature = retriever_convert_example_to_feature(passage_example, self._tokenizer, 
                                                                    max_length=self._passage_max_seq_length)
            negative_feature_dict = {'negative_input_ids': np.asarray(passage_feature.input_ids), 
                                'negative_token_type_ids': np.asarray(passage_feature.token_type_ids), 
                                'negative_attention_mask': np.asarray(passage_feature.attention_mask),
                                'negative_id': example_id}
            return_feature_dict.update(negative_feature_dict)
...
def retriever_convert_example_to_feature(example, tokenizer,
    ...
    inputs = tokenizer.encode_plus(
        ...
        # truncation = True
        #2022、4、5 3.4 transformers 增加truncation=True
    )
